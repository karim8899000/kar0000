...
admin_chat_id = ida
user_data = {}
TOKEN = "7078228710:AAEixs-J6dd3Dil9wt6ondq9DcaoCEQdrCo"
token = TOKEN
dev = "H5_H_H"
admin = ida
...
we10="https://t.me/Legends_Team_20600/1800"
face="https://t.me/Legends_Team_20600/1828"
yout="https://t.me/Legends_Team_20600/1829"
...
def callback_query(call):
    ...
    if call.data == 'open_menu':
     ...
     bot.send_message(call.message.chat.id, "قسم الفلكسات", reply_markup=menu_keyboard)
        ...
        bot.send_message(call.message.chat.id, "ارسل رابط الفيديو")
...
def rash3(message,link):
    ...
    s = 'ZSEDAYADFGHBCKEOSMASLAytadnxcvljsfgdapsoaslasdfaysfnbcnxzmbvzxcmvjz'
    f=message.text
    username = ''.join(random.choice(s) for _ in range(10))
    print(username)
    ...
    try:
        ...
        bot.reply_to(chat_id, "حدث خطأ أثناء محاولة الرشق.")
...
def activate_free_internet(user_data, message):
    try:
        ...
        bot.reply_to(message, "المحاولة في وقت لاحق 🚸")
...
def insta(message):
    ...
    bot.reply_to(message, "جاري التنزيل")
    success = download_instagram_video(post_url, download_path)
    
    if success:
        video_file = next((f for f in os.listdir(download_path) if f.endswith('.mp4')), None)
        if video_file:
            ...
            bot.reply_to(message, "تم التحميل ولكن خطأ ف الملف")
    else:
        bot.reply_to(message, "الرابط غير معروف")
def download_instagram_video(post_url, output_path='.'):
    loader = instaloader.Instaloader(dirname_pattern=output_path)
    
    try:
        shortcode = post_url.split("/")[-2]
        
        # Download the post
        print(f"Downloading Instagram video from '{post_url}' to '{output_path}'...")
        loader.download_post(instaloader.Post.from_shortcode(loader.context, shortcode), target=output_path)
